...
namespace Practifly.Checkers
{
    public class Checker
    {
        ...
        {
            foreach (var substring in _checkerOptions.SkipSubstrings)
            {
                properties.RemoveAll(e => e.Name.Contains(substring));
            }
        }

        private void SkipByEndWith(List<PropertyInfo> properties)
        {
            foreach (var endWith in _checkerOptions.SkipEndsWith)
            {
                properties.RemoveAll(e => e.Name.EndsWith(endWith));
            }
        }
        ...
        {
            foreach (var type in _checkerOptions.SkipTypes)
            {
                properties.RemoveAll(e => e.PropertyType == type);
            }
        }
